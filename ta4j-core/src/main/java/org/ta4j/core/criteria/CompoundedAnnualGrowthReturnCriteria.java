...
public class CompoundedAnnualGrowthReturnCriteria extends AbstractAnalysisCriterion {
  ...
  private int getHoldingPeriod(BarSeries series) {
    int startingYear = series.getFirstBar().getEndTime().atZone(ZoneId.systemDefault()).getYear();
    int endingYear = series.getLastBar().getEndTime().atZone(ZoneId.systemDefault()).getYear();
